 ...
 * Рассматрим строку S = "abracadabra", ее префкис- и Z-функцию:
 * Z:  # 0 0 1 0 1 0 4 0 0 1
 *     A B R A c a d A B R A
 * Pi: 1 0 0 1 0 1 0 1 2 3 4
 * 
 * Можно заметить, что в обоих функциях пик достигается при сравнении с префиксом
 * "ABRA", которому соответствуют Z[7] = 4 и Pi[10] = 4.
 * Видно, что в точке пика Z-функции префикс-функция начинает расти вплоть до равного пика.
 * 
 * Таким образом можно преобразовать Z-функцию в префикс-функцию: Pi[i...i + Z[i]] = j[0...z[i]]
 * Следует обратить внимание при реализации, что если значение префикс-функции уже установлено, то
 * оно неизменяемо, дабы избежать уменьшения. 
 * В крайнем случае можно брать max от прошлого и нового значения.
 * 
 * Ввод: Z - Z-функция некоторой строки
 * Вывод: Pi - Префикс-функция некоторой строки
 */
std::vector<int> prefix_from_z(std::vector<int> z) {
